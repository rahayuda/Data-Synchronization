...
file_path = 'D:/Python/MultiNodeDatabase/ubikuitas-firebase-adminsdk-px7rs-e2cb487e1b.json'
...
def fetch_existing_backup_ids():
    ref = db.reference('customerbackup')
...
def fetch_and_update_data(existing_backup_ids):
    ...
    try:
        with con.cursor() as cursor:
            query = "SELECT * FROM customerfullview"  # Mengambil data dari view
            cursor.execute(query)
            results = cursor.fetchall()
            for row in results:
                # Konversi Decimal menjadi float
